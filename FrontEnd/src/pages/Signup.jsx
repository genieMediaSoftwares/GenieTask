...
//       // Store employeeId in displayName for employee role: "name|role|employeeId"
//       const displayName =
//         role === "employee"
//           ? `${name}|${role}|${employeeId.trim()}`
//           : `${name}|${role}`;
...
//             {/* Employee ID — only shown for Employee role */}
//             {isEmployee && (
//               <div>
//                 <label className="block text-xs font-medium text-slate-500 mb-1.5">
//                   Employee ID
...
//                   Ask your admin for your Employee ID
//                 </p>
//               </div>
//             )}
...
//                 <button
//                   type="button"
//                   onClick={() => { setRole("admin"); setEmployeeId(""); }}
//                   className={`flex items-center justify-center gap-2 py-2.5 sm:py-3 px-3
//                     rounded-xl border text-sm font-medium transition-all active:scale-95
//                     ${role === "admin"
//                       ? "border-violet-500 bg-violet-50 text-violet-700 shadow-sm shadow-violet-100"
...
export default function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // Store name|role|employeeId in displayName for quick client reads
      ...
      // ── 2. Write to Realtime DB → users/${uid} ────────────────────
      //    Used by: AdminDashboard, AdminTasks, AdminAttendance, AdminLeave
      ...
      // ── 3. Write to Realtime DB → teamMembers/ ────────────────────
      //    Used by: TeamMembers panel, AdminDashboard roster, Sidebar badge
      //    Check for duplicate employeeId first
