...
// import { ref, onValue } from "firebase/database";
// import { useNavigate } from "react-router-dom";
// import { FaIdBadge, FaCheckCircle, FaClock, FaExclamationCircle } from "react-icons/fa";
...
//           )}
//           {profile.department && (
//             <span className="text-xs text-slate-400 hidden sm:inline">{profile.department}</span>
//           )}
//           {profile.designation && (
//             <span className="text-xs text-slate-500 font-medium hidden sm:inline">{profile.designation}</span>
//           )}
//         </div>
//         <div className="flex items-center gap-2">
...
//       {/* ── Greeting ── */}
//       <h1 className="text-2xl font-bold text-slate-800 mb-0.5">Good morning, {firstName} 👋</h1>
//       <p className="text-slate-400 text-sm mb-6">{today}</p>
...
//           { label:"DAYS PRESENT", value: totalDays > 0 ? `${presentDays}/${totalDays}` : "—",
//             sub: totalDays > 0 ? `${attendancePct}% attendance` : "No records yet",
//             color:"border-l-purple-400", icon:"📍", val_color:"text-purple-500" },
//         ].map(s => (
...
//                 <p className={`text-2xl font-bold ${s.val_color}`}>{s.value}</p>
...
//             <div className="flex items-center justify-between mb-4">
//               <div>
//                 <h3 className="font-semibold text-slate-800">My Assigned Tasks</h3>
//                 <p className="text-xs text-slate-400 mt-0.5">Tasks assigned by your admin</p>
//               </div>
//               <button onClick={() => navigate("/employee/tasks")} className="text-xs text-blue-500 hover:underline">
//                 View all →
//               </button>
//             </div>
...
//             <div className="flex items-center justify-between mb-4">
//               <h3 className="font-semibold text-slate-800">Recent Attendance</h3>
//               <button onClick={() => navigate("/employee/attendance")} className="text-xs text-blue-500 hover:underline">
//                 Details →
//               </button>
//             </div>
//             {attendance.length === 0 ? (
...
//                 {attendance.map(rec => (
//                   <div key={rec.date} className="flex items-center gap-3 text-sm">
//                     <span className="text-slate-600 w-28 shrink-0 text-xs">{formatDateLabel(rec.date)}</span>
//                     <span className="text-slate-400 flex-1 text-xs font-mono">
...
//               <button onClick={() => navigate("/employee/leave")} className="text-xs text-blue-500 hover:underline">
//                 View all →
//               </button>
//             </div>
//             {leaves.length === 0 ? (
...
//               <div className="divide-y divide-slate-50">
//                 {leaves.slice(0, 3).map(leave => {
//                   const badge = leave.status === "Approved" ? "bg-green-100 text-green-700"
//                     : leave.status === "Rejected" ? "bg-red-100 text-red-600"
//                     : "bg-yellow-100 text-yellow-700";
//                   return (
//                     <div key={leave.id} className="flex items-center justify-between py-2 first:pt-0 last:pb-0 text-sm">
//                       <div>
//                         <span className="text-slate-700 font-medium">{leave.type}</span>
//                         <span className="text-slate-400 text-xs ml-1.5">{leave.from} – {leave.to}</span>
//                       </div>
...
//               <div className="divide-y divide-slate-50">
//                 {recentlyCompleted.map(task => (
//                   <div key={task.id} className="flex items-center gap-2 py-2 first:pt-0 last:pb-0 text-sm">
...
// Map admin status values to employee-friendly display
...
export default function EmployeeDashboard() {
  ...
  useEffect(() => {
    ...
    // ─────────────────────────────────────────────────────────────
    // Read assigned tasks from flat "assignedTasks" collection,
    // filtering by assignedTo === current user's uid.
    // This matches exactly what AdminTasks.jsx writes.
    // ─────────────────────────────────────────────────────────────
    unsubs.push(
      onValue(ref(db, "assignedTasks"), (snap) => {
        const data = snap.val() || {};
    ...
    unsubs.push(
      onValue(ref(db, `leaveBalance/${user.uid}`), (snap) => {
        if (snap.val()) setLeaveBalance(snap.val());
      })
    );

    unsubs.push(
      onValue(ref(db, `leaves/${user.uid}`), (snap) => {
        const data = snap.val() || {};
        setLeaves(Object.entries(data).map(([id, v]) => ({ id, ...v })));
      })
    );
    ...
    <div className="p-6 max-w-[1200px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {profile.department  && <span className="text-xs text-slate-400 hidden sm:inline">{profile.department}</span>}
          {profile.designation && <span className="text-xs text-slate-500 font-medium hidden sm:inline">{profile.designation}</span>}
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate("/employee/attendance")}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          },
          {
            ...
            icon: "⏳",
            val_color: "text-orange-500",
          },
          {
            label: "DAYS PRESENT",
            value: totalDays > 0 ? `${presentDays}/${totalDays}` : "—",
            sub: totalDays > 0 ? `${attendancePct}% attendance` : "No records yet",
            color: "border-l-purple-400",
            icon: "📍",
            val_color: "text-purple-500",
          },
        ].map(s => (
          <div key={s.label} className={`bg-white rounded-xl p-4 border-l-4 ${s.color} shadow-sm`}>
            <div className="flex justify-between items-start">
            ...
            {recentlyCompleted.length === 0 ? (
              ...
              <div className="divide-y divide-slate-50">
                {recentlyCompleted.map(task => (
                  <div
                    key={task.id}
                    className="flex items-center gap-2 py-2 first:pt-0 last:pb-0 text-sm"
                  >
                    <FaCheckCircle className="text-green-500 shrink-0 text-xs" />
                    <span className="text-slate-400 line-through truncate">{task.title}</span>
                  </div>
